...
**v1.2 follow-up (2026-08-03) — verified-test provision dropped after the agreement measurement.** Human–LLM agreement on the val reference: **32%, Cohen's κ = 0.15** — the LLM tagger over-calls `mixed` (55/109) and `noise` (16/109 against a human count of zero) and is unusable as an annotator. Consequences, decided before any test tagging: tag-prompt v1 is **ratified as a measured negative result** (no v2 — iterating the prompt against the only human reference set would overfit the instrument, the same trap declined for the judge prompt on 2026-07-22); provision 3 above is dropped and the test seeded-50 is tagged **cold under the v1.1 procedure**, identical to val; all taxonomy analyses use human tags only; the LLM tag files remain committed as the negative-result record.
